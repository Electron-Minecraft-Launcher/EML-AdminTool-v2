...
class DataBase {
  ...
  async dbGenerate(tables: DBGeneration): Promise<void> {
    ...
    var isDb: count
    const t = Object.entries(tables).filter(([, value]) => value === true).map(([key]) => key)

    try {
      isDb = (await this.query<count[]>('SELECT COUNT(*) as count FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = \'eml_admintool\''))[0]
    } catch (error: any) {
      throw new Error(error)
    }
